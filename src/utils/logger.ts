...
export class Logger {
	...
	static configureFileLogging(adapter: DataAdapter, pluginDir: string) {
		Logger.fileTarget = {
			adapter,
			path: normalizePath(`${pluginDir}/myhistory.log`)
		};

		Logger.enqueueFileWrite(`${formatTimestamp()} [info] [MyHistory:Logger] File logging started\n`);
	}
	...
	private write(level: LogLevel, message: string, context?: LogContext, error?: unknown) {
		...
		const prefix = `[MyHistory:${this.scope}] ${message}`;
	...
	private static enqueueFileWrite(line: string) {
		...
		Logger.writeQueue = Logger.writeQueue
			...
			.catch((error) => {
				console.error("[MyHistory:Logger] Failed to write log file", error);
			});
	}
}
...
function formatLogLine(
	...
	return `${formatTimestamp()} [${level}] [MyHistory:${scope}] ${message}${formatDetails(context, error)}\n`;
}
